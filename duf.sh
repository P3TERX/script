...
# File name: duf.sh
# Description: Install latest version duf
...
PROJECT_NAME='duf'
GH_API_URL='https://api.github.com/repos/muesli/duf/releases/latest'
BIN_NAME='duf'
...
case ${OS_ARCH} in
*86)
    FILE_KEYWORD='linux_i386'
    ;;
x86_64 | amd64)
    FILE_KEYWORD='linux_x86_64'
    ;;
aarch64 | arm64)
...
DOWNLOAD_URL=$(curl -fsSL ${GH_API_URL} | grep 'browser_download_url.*tar.gz' | cut -d'"' -f4 | grep "${FILE_KEYWORD}")
